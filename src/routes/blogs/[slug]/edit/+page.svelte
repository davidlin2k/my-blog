...
<label for="cover-image-input" class="hover:cursor-pointer">
	<img
		class="w-screen h-[30vh] object-cover"
		src={isEmptyString(coverImage) ? placeholder : coverImage}
		alt="Cover"
	/>
</label>
...
<div class="max-w-3xl px-8 py-4 m-auto">
	<div class="flex items-center mb-3">
		<Breadcrumb>
			...
			>
			<BreadcrumbItem
				linkClass="ml-1 text-sm font-medium text-text-100 max-w-[128px] overflow-hidden whitespace-nowrap overflow-ellipsis hover:text-text-200 md:ml-2"
				href={`/blogs/${data.id}`}>{isEmptyString(title) ? 'Untitled' : title}</BreadcrumbItem
			>
			<BreadcrumbItem linkClass="ml-1 text-sm font-medium text-text-100 hover:text-text-200 md:ml-2"
				>Edit</BreadcrumbItem
			>
		</Breadcrumb>
	...
	<DivInput
		className="block text-3xl md:text-4xl py-4 font-semibold bg-transparent border-none focus:ring-0 focus:outline-none"
		placeholder="Untitled"
		bind:value={title}>{title}</DivInput
	>
			...
			<tbody>
				...
				<tr class="items-center text-text-200">
					...
					<td>
						<Select
							id="countries"
							class="p-0 bg-transparent border-0 focus:ring-0 text-text-100 inline-block w-auto"
							defaultClass=""
							bind:value={status}
	...
	<Editor data={data.content} {uploadByFile} bind:this={editorComponent} on:save={onSave} />
</div>
